...
OUTPUT_DATASET = Path(r"E:\Education\4 course 2 semester\Diploma\panoptic_project\data\LaRS_advanced")
...
def sanitize_keypoints(keypoints, img_w, img_h, eps=1e-3):
    sanitized = []
    for kp in keypoints:
        kp = list(kp)
        kp[0] = min(max(float(kp[0]), 0.0), img_w - eps)
        kp[1] = min(max(float(kp[1]), 0.0), img_h - eps)
        sanitized.append(tuple(kp))
    return sanitized
...
def copy_original_dataset(source_root: Path, output_root: Path):
    for split in ["train", "val"]:
...
def save_yaml(output_root: Path):
    yaml_path = output_root / "LaRS_advanced.yaml"
    lines = [
...
def augment_train_only(source_root: Path, output_root: Path):
    ...
    for label_path in sorted(labels_dir.glob("*.txt")):
        ...
        for inst_idx, line in enumerate(lines):
            ...
            cls_id, xs, ys = parsed
            n_aug = AUGS_PER_INSTANCE.get(cls_id, 0)
            ...
            for aug_idx in range(n_aug):
                ...
                out_img_name = f"{label_path.stem}_crop_{cls_id}_{inst_idx}_aug{aug_idx}{suffix}"
                out_lbl_name = f"{label_path.stem}_crop_{cls_id}_{inst_idx}_aug{aug_idx}.txt"
                ...
                if ok:
                    created_per_class[cls_id] += 1

    return created_per_class


def count_instances_in_split(root: Path, split: str):
...
def main():
    ...
    if not SOURCE_DATASET.exists():
        raise FileNotFoundError(
            ...
            f"Положи папку 'instance_yolo' рядом со скриптом или укажи правильный путь в SOURCE_DATASET."
        )

    ensure_structure(OUTPUT_DATASET)

    print("1) Копирую исходный датасет...")
    copy_original_dataset(SOURCE_DATASET, OUTPUT_DATASET)

    print("2) Считаю исходное число инстансов в train...")
    before = count_instances_in_split(SOURCE_DATASET, "train")
    for cls_id in sorted(CLASS_NAMES.keys()):
        print(f"  {cls_id} ({CLASS_NAMES[cls_id]}): {before.get(cls_id, 0)}")

    print("\n3) Добавляю умеренное число новых кропов для редких классов...")
